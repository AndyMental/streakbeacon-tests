...
deployed site; do not import or inspect app source.

StreakBeacon is local-first with no authentication surface (per AND-5634 /
AND-5288). All required state is established through the public UI on a fresh
browser context; no seeded server-side account is required.
...
| --- | --- | --- | --- | --- | --- | --- |
| User marks today's streak day from the deployed site | `features/login-mark-today.feature` | `tests/smoke/login-mark-today.spec.ts` (skipped — pending local-first rewrite) | `STREAKBEACON_BASE_URL` | `npm run test:smoke` | The user opens the deployed site, marks today if needed, and the today control is visibly marked. | Blocked: scenario and spec were written against an assumed auth surface that does not exist (per AND-5634); needs rewrite for a local-first first-run flow before it can execute. Spec currently skips at runtime so the smoke harness does not fail. |
...
- Execute `npm run test:smoke` against the deployed Vercel URL.
- Rewrite the mark-today smoke scenario and spec for the local-first model
  (open deployed site → reach today's streak control through the public
  first-run UI → mark today) and confirm the public mark-today control can be
  located on the deployed site.
