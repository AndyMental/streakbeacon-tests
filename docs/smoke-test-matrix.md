...
deployed site and local browser state; do not import or inspect app source.

StreakBeacon is local-first with no authentication surface for the current MVP.
All required state is established through the public UI in a fresh browser
context; seeded server-side accounts are not required unless a later issue
records an approved authentication surface.
...
| --- | --- | --- | --- | --- | --- | --- |
| User marks today's habit complete | `features/today-streak-completion.feature` | `tests/smoke/login-mark-today.spec.ts` is suite-skipped because it still contains the older seeded-login flow and needs a local-first first-run rewrite before this scenario is automated | `STREAKBEACON_BASE_URL` and a deterministic local-first state setup strategy | Pending automation after Gherkin validation and state setup strategy | Today's grid cell for `Read` is shown as completed and current streak count is `1`. | Browser verification is blocked until a deployed URL and approved local-first state strategy are supplied. |
...
- Rewrite the mark-today smoke scenario and spec for the local-first model:
  open the deployed site, create or locate a habit through the public first-run
  UI, mark today, and assert the visible completed state.
