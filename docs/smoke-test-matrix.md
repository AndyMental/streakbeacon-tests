...
| ---------------------------------------------------- | -------------------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL` | Issue metadata key `deploy_url` or release handoff | Every Playwright run      | Must be a deployed Vercel URL, not localhost. `STREAKBEACON_BASE_URL` takes precedence when more than one is set. |
...
```bash
DEPLOY_URL=<deployed-vercel-url> \
...
| --------------------------------------------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| User marks today's habit complete                                     | `features/today-streak-completion.feature`      | `tests/smoke/login-mark-today.spec.ts` currently covers the older seeded-login smoke path and needs local-first refactor before this scenario is automated | Deployed URL env (`STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL`) and a deterministic local-first state setup strategy                                                    | Pending automation after Gherkin validation and state setup strategy                      | Today's grid cell for `Read` is shown as completed and current streak count is `1`.                                 | Browser verification is blocked until a deployed URL and approved local-first state strategy are supplied.                                          |
| User adds a first habit and sees a zero streak                        | `features/create-habit.feature`                 | Pending                                                                                                                                                    | Deployed URL env (`STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL`) and a deterministic clean or resettable local store                                                     | Pending automation after Gherkin validation and state setup strategy                      | The created habit appears in the visible list with streak count `0`.                                                | Selector discovery, state setup/reset, and browser verification are blocked until a deployed URL and approved test-state strategy are supplied.     |
| Habit with 7 contiguous marked days shows a filled recent-history row | `features/dashboard-streak-grid-render.feature` | Pending                                                                                                                                                    | Deployed URL env (`STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL`) and deterministic local data containing a habit named `Read` with 7 contiguous marked days ending today | Pending automation after deterministic data setup is available                            | Dashboard shows 7 filled recent-history cells and current streak count `7`.                                         | State setup verification, visual color assertion, and browser verification are blocked until deployed URL and fixture details are supplied.         |
| User sees loading status while app content is pending                 | `features/app-shell-resilience.feature`         | Pending                                                                                                                                                    | Deployed URL env (`STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL`) and an approved delayed-content route or test fixture                                                   | Pending automation after Gherkin validation and an observable delayed state are available | A public loading affordance is exposed as `role=status`, announces StreakBeacon loading, and does not trap focus.   | Browser verification is blocked until implementation exposes a deterministic delayed route or naturally observable pending state on a deployed URL. |
| User can recover from an error fallback                               | `features/app-shell-resilience.feature`         | Pending                                                                                                                                                    | Deployed URL env (`STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL`) and an approved error-producing route or test fixture                                                   | Pending automation after Gherkin validation and deterministic error trigger are available | The fallback exposes a visible error alert and reset control.                                                       | Browser verification is blocked until implementation exposes a deterministic error route or recoverable failing state on a deployed URL.            |
| User retries after an error fallback                                  | `features/app-shell-resilience.feature`         | Pending                                                                                                                                                    | Deployed URL env (`STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL`) and an approved recoverable error state                                                                 | Pending automation after reset behavior is observable from the public UI                  | Activating reset reloads or retries the app view and removes the error alert after recovery succeeds.               | Browser verification is blocked until a recoverable public error state is available.                                                                |
| Keyboard user skips directly to main content                          | `features/app-shell-resilience.feature`         | Pending                                                                                                                                                    | Deployed URL env (`STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL`)                                                                                                         | Pending automation after skip-link Gherkin validation                                     | Pressing Tab once focuses a visible "Skip to main content" link.                                                    | Browser verification is blocked until a deployed URL for the branch is supplied.                                                                    |
| Keyboard user activates the skip link                                 | `features/app-shell-resilience.feature`         | Pending                                                                                                                                                    | Deployed URL env (`STREAKBEACON_BASE_URL`, `DEPLOY_URL`, or `BASE_URL`)                                                                                                         | Pending automation after skip-link Gherkin validation                                     | Activating the skip link moves focus to the main landmark and updates the location hash to the main content target. | Browser verification is blocked until a deployed URL for the branch is supplied.                                                                    |
